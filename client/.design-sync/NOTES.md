...
Preferred — the resync driver (build → diff → validate → capture in one verdict). Fetch `_ds_sync.json` from the project first (`DesignSync(get_file)`) into `.design-sync/.cache/remote-sync.json`, then from `client/`:
```bash
node .ds-sync/resync.mjs --config .design-sync/config.json --node-modules ./node_modules --entry ./.ds-entry.mjs --out ./ds-bundle --remote .design-sync/.cache/remote-sync.json
```

Manual fallback (build + validate only, no diff/anchor):
```bash
...
- **Re-synced 2026-07-01 via `resync.mjs`.** All 8 source keys matched the project's anchor (`_ds_sync.json`) — no source changed since the last sync, `upload.any: false`, nothing re-uploaded. Render check was skipped by the driver itself on this no-change path (`[RENDER_SKIPPED]` — expected, not a new warn). As part of this run, validated every claim in `conventions.md` against the actual component source (the standing "keeping it true" duty) and found three real drifts, now fixed: `Logo`'s documented default `size` was 28 (actual: 26) and `variant` enum was `"default"|"splash"` (actual: `"brand"|"splash"`, default `"brand"`) — also added the undocumented `radius` override prop; `GoalProgressBar` was missing its `color` prop (default `var(--accent)`, overridable for warn/danger) and the doc incorrectly said it fills `--brand` indigo (actually `--accent` green by default); `GoalCard` documented a `subtitle` prop that doesn't exist — the real prop is `summary`. All other component/prop/token claims in the file re-verified clean against source and the fresh `ds-bundle/` output.
